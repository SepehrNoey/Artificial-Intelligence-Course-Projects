...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        if state[1][0] == "unvisited" or state[1][1] == "unvisited" or\
           state[1][2] == "unvisited" or state[1][3] == "unvisited":
            return False
        return True
...
def cornersHeuristic(state, problem):
    ...
    maxHeu = 0
    for i in range(4):
        if(state[1][i] == "unvisited"):
            dist = manhattanDistance(state[0], corners[i])
            maxHeu = dist if dist > maxHeu else maxHeu
    return maxHeu
...
def foodHeuristic(state, problem):
    ...
    for food in food_list:
        foodDistance = mazeDistance(state[0], food, problem.startingGameState)
        if foodDistance > heuristic: # searching for farthest node
