...
class CornerProblemTest(testClasses.TestCase):
    ...
    def solution(self, search, searchAgents):
        ...
        path = search.dfs(problem)  ## change
        print(path)
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        rev_gold_length = int(solutionDict['rev_solution_length'])
        ...
        if len(solution) != gold_length and len(solution) != rev_gold_length:
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('Specified solution not found.')
            grades.addMessage('\tstudent solution length:\n%s' % len(solution))
            grades.addMessage('')
            grades.addMessage('\tcorrect solution length:\n%s' % gold_length)
            grades.addMessage('')
            grades.addMessage('\tcorrect reverse solution length:\n%s' % rev_gold_length)
            return False
